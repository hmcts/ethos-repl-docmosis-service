...
class MigrateToReformServiceTest {
    ...
    void migrateToReform() throws IOException {
        ...
        verify(ccdClient, times(1)).retrieveCase(AUTH_TOKEN, ecmCaseDetails.getCaseTypeId(), "EMPLOYMENT", ecmCaseDetails.getCaseId());
        verify(ccdClient, times(1)).startCaseMigrationToReform(AUTH_TOKEN, "EMPLOYMENT", "ET_EnglandWales");
    ...
    void unknownCaseType() throws IOException {
        ...
        assertThrows(IllegalArgumentException.class, () -> migrateToReformService.migrateToReform(AUTH_TOKEN, ecmCaseDetails));
        verify(ccdClient, times(0)).retrieveCase(AUTH_TOKEN, ecmCaseDetails.getCaseTypeId(), "EMPLOYMENT", ecmCaseDetails.getCaseId());
        verify(ccdClient, times(0)).startCaseMigrationToReform(AUTH_TOKEN, "EMPLOYMENT", "ET_EnglandWales");
    }
    ...
    private uk.gov.hmcts.et.common.model.ccd.CaseDetails generateReformCaseDetails(String jsonFileName) throws Exception {
        String json = new String(Files.readAllBytes(Paths.get(Objects.requireNonNull(getClass().getClassLoader()
                .getResource(jsonFileName)).toURI())));
        ObjectMapper mapper = new ObjectMapper();
