...
@Service
@Slf4j
public class HearingsByHearingTypeReportOld {
                ...
                for (var dateListedTypeItem : hearingTypeItem.getValue().getHearingDateCollection()) {
                    if (checkIfValidHearing(dateListedTypeItem.getValue())) {
                        var date = LocalDateTime.parse(
                                dateListedTypeItem.getValue().getListedDate(), NEW_DATE_PATTERN).toLocalDate().toString();
                        datesList.add(date);
                    }
                }
            }
    ...
    private Set<String> getDate(String date, List<String> datesList) {
        var date1 = LocalDateTime.parse(date, NEW_DATE_PATTERN).toLocalDate().toString();
        datesList.add(date1);
        return datesList;
    }
            ...
            if (CollectionUtils.isNotEmpty(caseData.getHearingCollection())) {
                for (HearingTypeItem hearingTypeItem : caseData.getHearingCollection()) {
                   if (CollectionUtils.isNotEmpty(hearingTypeItem.getValue().getHearingDateCollection())) {
                      setFields(hearingTypeItem, fields);
                   }
                }
            }
        }
    ...
    private void setFields(HearingTypeItem hearingTypeItem, Fields fields) {
    ...
    private void setSummaryFields(DateListedTypeItem dateListedTypeItem, String hearingType, Fields fields) {
        String date = getDate(dateListedTypeItem.getValue().getListedDate());
    }

    private void calculateLocalReportSummaryHdr1(List<SubmitEvent> submitEventList, Fields fields) {
            ...
            fields.costs = (int) submitEventList.stream().filter(a -> CollectionUtils.isNotEmpty(
                    a.getCaseData().getHearingCollection())
                    && a.getCaseData().getHearingCollection().stream().anyMatch(
                        b -> COSTS_HEARING_TYPE.equals(b.getValue().getHearingType())
                                && b.getValue().getHearingDateCollection().stream().anyMatch(
                                    c -> checkIfValidHearing(c.getValue())
                        ))).count();
            ...
            fields.costs = (int) submitEventList.stream().filter(a -> CollectionUtils.isNotEmpty(
                    ...
                    && a.getCaseData().getHearingCollection().stream().anyMatch(
                        b -> CollectionUtils.isNotEmpty(b.getValue().getHearingDateCollection())
                            && b.getValue().getHearingDateCollection().stream().anyMatch(c -> date.equals(
                            c.getValue().getListedDate()))
                            && COSTS_HEARING_TYPE.equals(b.getValue().getHearingType()))).count();
            fields.reconsider = (int) submitEventList.stream().filter(a -> CollectionUtils.isNotEmpty(
                    a.getCaseData().getHearingCollection())
                    && a.getCaseData().getHearingCollection().stream().anyMatch(
                        b -> CollectionUtils.isNotEmpty(b.getValue().getHearingDateCollection())
                ...
                fields.costs =  (int) submitEventList.stream().filter(a -> CollectionUtils.isNotEmpty(
                        ...
                        && a.getCaseData().getHearingCollection().stream().anyMatch(
                            b -> CollectionUtils.isNotEmpty(b.getValue().getHearingDateCollection())
                                && b.getValue().getHearingDateCollection().stream().anyMatch(c -> date.equals(
                                c.getValue().getListedDate()))
                                && COSTS_HEARING_TYPE.equals(b.getValue().getHearingType())
                                && isHearingFormatValid(subSplitHeader, b))).count();
                fields.reconsider =  (int) submitEventList.stream().filter(a -> CollectionUtils.isNotEmpty(
                        a.getCaseData().getHearingCollection())
                        && a.getCaseData().getHearingCollection().stream().anyMatch(
