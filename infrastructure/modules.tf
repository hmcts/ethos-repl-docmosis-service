data "azurerm_user_assigned_identity" "ethos-shared-identity" {
  name                = "ethos-shared-${var.env}-mi"
...
module "key-vault" {
  ...
  managed_identity_object_ids = ["${data.azurerm_user_assigned_identity.ethos-shared-identity.principal_id}"]
}
